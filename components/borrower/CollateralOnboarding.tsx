...
export function CollateralOnboarding({ vaultId, loanId, defaultCollateralUsd }: Props) {
      ...
      <div className="rounded-xl border border-white/30 bg-white/[0.09] backdrop-blur-md p-5 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          <div className="space-y-2">
            <div className="text-sm uppercase tracking-wider text-white/75">dWallet ID</div>
          ...
          <div className="space-y-2">
            <div className="text-sm uppercase tracking-wider text-white/75">Chain</div>
            <div className="text-sm text-white/80">{CHAIN_LABELS[collateral.chainId as IkaChain]}</div>
          </div>

          <div className="space-y-2">
            <div className="text-sm uppercase tracking-wider text-white/75">Collateral (USD)</div>
          ...
          <div className="space-y-2">
            <div className="text-sm uppercase tracking-wider text-white/75">Oracle</div>
            <div className="font-mono text-xs text-white/85 truncate" title={collateral.oraclePubkey.toBase58()}>
        ...
        <div className="pt-4 border-t border-white/5">
          {collateral.lockedTs > 0 && (
            <div className="flex items-center justify-between text-xs">
              <span className="text-white/75">Locked at</span>
              <span className="text-white/65">{new Date(collateral.lockedTs * 1000).toLocaleString()}</span>
            </div>
          )}
        </div>
        ...
        {collateral.status === 'Pending' && (
          <>
            ...
            <div className="rounded-xl border border-yellow-500/20 bg-yellow-500/[0.03] p-4 space-y-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 text-sm uppercase tracking-wider font-semibold text-yellow-500">
                  <div className="h-1.5 w-1.5 rounded-full bg-yellow-500 animate-pulse" />
                  Phase 3 — Funding & Monitoring
                </div>
                {isFetchingAddress && <div className="h-3.5 w-3.5 animate-spin rounded-full border border-yellow-500/20 border-t-yellow-500" />}
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-2">
                <div className="flex-1 space-y-3">
                  ...
                  <div className="space-y-3">
                    <div className="text-sm uppercase tracking-wider text-white/65 font-medium">Deposit Address</div>
                    {depositAddressError ? (
                      <div className="rounded-lg border border-yellow-500/20 bg-yellow-500/[0.05] p-4 space-y-3">
                        ...
                        {depositAddressError.includes('NetworkRejectedDKGVerification') && (
                          <button
                            ...
                            className="flex items-center gap-2 rounded-lg bg-yellow-500/20 px-3 py-2 text-sm font-bold uppercase tracking-wider text-yellow-500 hover:bg-yellow-500/30 transition-all disabled:opacity-50"
                          >
                            {isCreatingDwallet ? (
                              <div className="h-3.5 w-3.5 animate-spin rounded-full border-2 border-yellow-500/20 border-t-yellow-500" />
                            ) : (
                              'Re-create dWallet'
                            )}
                          </button>
                      ...
                      <div className="flex items-center gap-2 rounded-lg border border-white/5 bg-black/20 p-3">
                        ...
                        <button
                          ...
                          className="p-1.5 text-white/85 hover:text-yellow-500 transition-colors disabled:opacity-30"
                        >
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between text-sm uppercase tracking-wider">
                      <span className="text-white/65 font-medium">Monitoring Status</span>
                      <span className="text-yellow-500 animate-pulse">Active</span>
                    </div>
                    <div className="space-y-2.5">
                      <Progress value={isPolling ? 66 : 33} className="h-1.5 bg-white/10" />
                      <div className="flex justify-between text-xs text-white/85 font-mono">
                ...
                {depositAddress && !depositAddressError && (
                  <div className="shrink-0 flex flex-col items-center gap-2">
                    <div className="p-2.5 rounded-xl bg-white border border-white/30 shadow-xl shadow-black/50">
                      <QRCodeCanvas
                    ...
                    <span className="text-xs text-white/65 uppercase tracking-widest font-semibold mt-2">Scan to Fund</span>
                  </div>
                )}
              </div>
            </div>
            ...
            <button
              ...
              className="w-full rounded-xl bg-purple-600 px-4 py-2 text-sm font-semibold text-white shadow-lg shadow-purple-900/20 hover:bg-purple-500 transition-all disabled:opacity-50"
            >
              {isPolling ? (
        ...
        {collateral.status === 'Locked' && (
          <div className="space-y-3 animate-in fade-in slide-in-from-bottom-3 duration-700">
            {/* ── Milestone Header ────────────────────────────────────────── */}
            <div className="rounded-sm border border-pink-400/20 bg-pink-400/[0.04] p-5">
              <div className="flex items-start gap-2">
                <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-sm bg-pink-400/10">
                  <BadgeCheck className="h-3.5 w-3.5 text-pink-400" />
                </div>
                <div className="flex-1">
                  <h3 className="text-lg font-bold text-white tracking-tight">Collateral successfully registered and secured.</h3>
                  <p className="mt-2 text-xs text-white/75 leading-relaxed max-w-xl">
                  ...
                  <div className="mt-2 flex items-center gap-2">
                    <div className="flex flex-col">
                      <span className="font-mono text-xs uppercase tracking-widest text-white/85">Custody Confirmation</span>
                      <span className="font-mono text-sm text-pink-400/80 mt-0.5">Verified by IKA Oracle</span>
                    </div>
                    <div className="flex flex-col">
                      <span className="font-mono text-xs uppercase tracking-widest text-white/85">Secured At</span>
                      <span className="font-mono text-sm text-white/80 mt-0.5">
            ...
            <div className="grid sm:grid-cols-2 gap-2">
              ...
              <div className="rounded-sm border border-white/[0.25] bg-white/[0.07] p-4 space-y-3">
                <div className="flex items-center justify-between pb-2 border-b border-white/[0.20]">
                  <div className="font-mono text-xs uppercase tracking-widest text-white/65">Facility Readiness</div>
                  <div className="flex items-center gap-2">
                    <div className="h-1 w-1 rounded-full bg-pink-400 animate-pulse" />
                    <span className="font-mono text-xs uppercase tracking-widest text-pink-400/70">Ready</span>
                  </div>
                </div>
                <div className="space-y-3">
                  {[
                    ...
                    <div key={idx} className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <item.icon className="h-3.5 w-3.5 text-white/85" />
                        <span className="font-mono text-xs uppercase tracking-widest text-white/65">{item.label}</span>
                      </div>
                      <span className={cn("font-mono text-sm", item.ok ? "text-pink-400/60" : "text-white/85")}>{item.value}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="rounded-sm border border-white/[0.25] bg-white/[0.07] p-4 space-y-3">
                <div className="font-mono text-xs uppercase tracking-widest text-white/65 pb-2 border-b border-white/[0.20]">Funding Pipeline</div>
                <div className="space-y-3 relative">
                  {[
                    ...
                    <div key={idx} className="flex items-center gap-2 relative z-10">
                      <div className={cn(
                        "flex h-3.5 w-3.5 shrink-0 items-center justify-center rounded-full border transition-all",
                        step.status === 'done' ? "border-pink-400/50 bg-pink-400/10 text-pink-400"
                        : step.status === 'active' ? "border-white/40 bg-white/10 text-white animate-pulse"
                      ...
                      <span className={cn(
                        ...
                        step.status === 'done' ? "text-pink-400/50" : step.status === 'active' ? "text-white/90" : "text-white/75"
                      )}>
                        {step.label}
                      </span>
                    </div>
            ...
            {!isDisbursed ? (
              <div className="rounded-sm border border-amber-500/15 bg-amber-500/[0.03] p-4">
                <div className="flex items-start gap-2">
                  <Clock className="h-3.5 w-3.5 text-amber-400/40 shrink-0 mt-0.5" />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-bold text-amber-200/80 uppercase tracking-wider">Disbursement Execution Pending</span>
                      <span className="font-mono text-xs text-amber-500/50">Next: Admin Action</span>
                    </div>
                    <p className="mt-2 text-xs leading-relaxed text-white/75 max-w-xl">
                    ...
                    <div className="mt-2 flex items-center gap-2 text-sm text-white/85">
                      ...
                      <span>Your collateral is now protecting the credit facility. Repayment controls will activate once the loan enters <span className="text-white/75 font-bold italic">Active</span> status.</span>
                    </div>
                  </div>
                </div>
              </div>
            ) : (
              <div className="rounded-sm border border-pink-400/15 bg-pink-400/[0.03] p-4">
                <div className="flex items-start gap-2">
                  <BadgeCheck className="h-3.5 w-3.5 text-pink-400/40 shrink-0 mt-0.5" />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-bold text-emerald-200/80 uppercase tracking-wider">Facility Fully Funded</span>
                      <span className="font-mono text-xs text-pink-400/50">Status: Active</span>
                    </div>
                    <p className="mt-2 text-xs leading-relaxed text-white/75 max-w-xl">
              ...
              <div className="mt-5 space-y-3">
                <div className="rounded-sm border border-white/[0.25] bg-white/[0.07] p-4">
                  <div className="flex items-center justify-between mb-3">
                    <span className="font-mono text-sm text-white/75 uppercase tracking-widest">Active Repayment</span>
                  ...
                  <div className="flex gap-2">
                    <div className="relative flex-1">
                      <input
                        ...
                        className="w-full rounded-sm border border-white/[0.28] bg-black/40 px-3 py-2.5 font-mono text-xs text-white placeholder:text-white/75 focus:border-pink-400/30 focus:outline-none transition-colors"
                      />
                      <span className="absolute right-3 top-1/2 -translate-y-1/2 font-mono text-xs text-white/85 uppercase">USDC</span>
                    </div>
                    <button
                      ...
                      className="rounded-sm bg-pink-400/80 px-4 py-2 text-sm font-bold uppercase tracking-widest text-emerald-950 hover:bg-pink-400 disabled:opacity-50 transition-all"
                    >
                      {repayMutation.isPending ? 'Processing…' : 'Repay'}
                    </button>
                  </div>

                  <div className="mt-2 flex items-center justify-between border-t border-white/[0.20] pt-4">
                    <div className="flex flex-col gap-0.5">
                      <span className="font-mono text-xs text-white/85 uppercase">Total Repaid</span>
                      <span className="font-mono text-xs text-pink-400/60">
                        ${loan ? (Number(loan.totalRepaid.toString()) / 1_000_000).toLocaleString() : '0.00'} USDC
                      </span>
                    </div>
                    <div className="flex flex-col items-center gap-0.5">
                      <span className="font-mono text-xs text-white/85 uppercase">Accrued Interest</span>
                    ...
                    <div className="flex flex-col items-end gap-0.5">
                      <span className="font-mono text-xs text-white/85 uppercase">Total Debt</span>
            ...
            {loanIsRepaid ? (
              <button
                ...
                className="w-full rounded-sm border border-blue-500/30 bg-blue-500/10 px-4 py-2 text-xs font-bold uppercase tracking-widest text-blue-400 hover:bg-blue-500/20 transition-all disabled:opacity-50"
              >
                {releaseMutation.isPending ? 'Releasing…' : 'Release Collateral'}
              </button>
            ) : (
              <div className="rounded-sm border border-white/[0.18] bg-white/[0.07] p-4 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <Shield className="h-3.5 w-3.5 text-white/75" />
                  <span className="font-mono text-sm text-white/85 uppercase tracking-widest">Facility Protection Active</span>
                </div>
                <div className="font-mono text-xs text-white/85 italic">
  ...
  if (isLoading) {
    return (
      <div className="rounded-xl border border-white/30 bg-white/[0.07] p-5 space-y-3 animate-pulse">
  ...
  return (
    <div className="rounded-xl border border-white/30 bg-white/[0.07] backdrop-blur-sm p-5 space-y-3">
      {/* Debug Info */}
      <div className="p-2 bg-black/40 rounded border border-white/5 font-mono text-xs text-white/85 mb-3 break-all">
      ...
      <div className="space-y-3">
        ...
        {!dwalletIdHex && (
          <div className="rounded-xl border border-purple-500/20 bg-purple-500/[0.03] p-4 space-y-3">
            <div className="flex items-center gap-2 text-sm uppercase tracking-wider font-semibold text-purple-400">
            ...
            {suiAddress ? (
              <div className="space-y-3">
                <div className="flex items-center gap-2 rounded-lg border border-white/30 bg-black/20 px-4 py-2.5">
                  <span className="flex-1 font-mono text-xs text-white/85 truncate">{suiAddress}</span>
                  <button
                    type="button"
                    onClick={handleCopyAddress}
                    className="shrink-0 text-white/85 hover:text-purple-400 transition-colors"
                    title="Copy address"
                  >
                  ...
                  <button
                    type="button"
                    onClick={() => disconnect()}
                    className="text-sm text-white/85 hover:text-red-400 transition-colors"
                  >
                    Disconnect
                  </button>
                </div>
                <div className="flex items-center gap-2 px-1">
                  <div className="flex items-center gap-2">
                    <span className="text-sm uppercase tracking-wider text-white/65">SUI</span>
                    <span className={`font-mono text-xs font-medium ${suiBalances && suiBalances.sui > 0 ? 'text-green-400' : 'text-white/75'}`}>
                      {suiBalances ? suiBalances.sui.toFixed(4) : '—'}
                    </span>
                  </div>
                  <div className="h-3 w-px bg-white/10" />
                  <div className="flex items-center gap-2">
                    <span className="text-sm uppercase tracking-wider text-white/65">IKA</span>
                    <span className={`font-mono text-xs font-medium ${suiBalances && suiBalances.ika > 0 ? 'text-purple-400' : 'text-white/75'}`}>
                      {suiBalances ? suiBalances.ika.toFixed(4) : '—'}
                    </span>
                  </div>
                  {(!suiBalances || (suiBalances.sui === 0 && suiBalances.ika === 0)) && (
                    <span className="ml-auto text-sm text-yellow-400/70">needs funding</span>
                  )}
                </div>
              </div>
            ) : (
                ...
                <div className="grid grid-cols-2 gap-2">
                  {wallets.map((wallet) => (
                    <button
                      ...
                      className="flex items-center justify-center gap-2 rounded-lg border border-white/30 bg-white/[0.09] px-3 py-2 text-xs font-medium text-white/90 hover:bg-white/[0.05] hover:text-white transition-all"
                    >
                      {wallet.icon && <img src={wallet.icon} alt="" className="h-3.5 w-3.5" />}
                      Connect {wallet.name}
                    </button>
                  ))}
                </div>
            ...
            <div className="flex gap-2">
              <a
                ...
                className="flex items-center justify-center gap-2 rounded-lg border border-white/30 bg-white/[0.09] px-4 py-2 text-xs font-medium text-white/90 hover:bg-white/[0.05] hover:text-white transition-all"
              >
              ...
              <button
                ...
                className="rounded-lg border border-yellow-500/30 bg-yellow-500/[0.08] px-3 py-2 text-xs font-medium text-yellow-400 hover:bg-yellow-500/[0.15] transition-all"
              >
                Simulate
              </button>
            </div>
        ...
        {dwalletIdHex && (
          <div className="flex items-center justify-between rounded-xl border border-green-500/20 bg-green-500/[0.03] px-4 py-2">
            <span className="flex items-center gap-2 text-xs font-medium text-green-400">
              <CheckCircle2 className="h-3.5 w-3.5" />
              IKA dWallet ready
            </span>
            <div className="flex items-center gap-2">
              <span className="font-mono text-xs text-green-400/60">{dwalletIdHex.slice(0, 12)}…</span>
              <button
                type="button"
                onClick={handleCreateDwallet}
                disabled={isCreatingDwallet}
                className="text-xs text-green-400/40 hover:text-green-400 hover:underline disabled:opacity-50 transition-colors"
              >
        ...
        {/* ── Register collateral ──────────────────────────────────────────── */}
        <div className="space-y-3">
          <div className="flex items-center gap-2 text-sm uppercase tracking-wider font-semibold text-white/75">
          ...
          <div className="grid sm:grid-cols-2 gap-2">
            <div className="space-y-3">
              <label className="block text-sm uppercase tracking-wider font-medium text-white/75">Chain</label>
            ...
            <div className="space-y-3">
              ...
              <div className="group relative">
                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-sm text-white/85 group-focus-within:text-purple-400">$</span>
                <input
                  type="number"
                  min="0"
                  value={collateralUsd}
                  ...
                  placeholder="50,000"
                  className="w-full rounded-xl border border-white/30 bg-white/[0.09] pl-8 pr-4 py-2 text-sm text-white focus:outline-none focus:ring-2 focus:ring-purple-500/40"
                />
              </div>
            </div>
          </div>
          ...
          {!dwalletIdHex && (
            <div className="space-y-3">
              ...
              <input
                ...
                className="w-full rounded-xl border border-white/30 bg-white/[0.09] px-4 py-2 font-mono text-xs text-white focus:outline-none focus:ring-2 focus:ring-purple-500/40"
              />
            </div>
          )}
          ...
          <div className="space-y-3">
            ...
            {oracleKey === TEST_ORACLE_PUBKEY && (
              <p className="text-sm text-purple-400/60 px-1 italic">Using local test oracle</p>
            )}
          </div>
        </div>
      </div>
      ...
      <button
        ...
        className="w-full rounded-xl bg-purple-600 px-4 py-2.5 text-sm font-bold text-white shadow-lg shadow-purple-900/20 hover:bg-purple-500 active:scale-[0.98] transition-all disabled:opacity-50"
      >
